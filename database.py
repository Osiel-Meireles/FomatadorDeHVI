...
def criar_tabelas():
    ...
    CREATE TABLE IF NOT EXISTS fardos (
        ...
        sci INTEGER,
        csp INTEGER,
        FOREIGN KEY (formatacao_id) REFERENCES formatacoes(id)
    )
    """)
    conn.commit()
    conn.close()

def inserir_formatacao(lote, data_hvi, safra, produtor):
    conn = conectar()
    cursor = conn.cursor()
    data_formatacao = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cursor.execute("""
        INSERT INTO formatacoes (lote, data_formatacao, data_hvi, safra, produtor)
        VALUES (?, ?, ?, ?, ?)
    """, (lote, data_formatacao, data_hvi, safra, produtor))
    conn.commit()
...
def listar_formatacoes():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT f.id, f.lote, f.safra, f.produtor, f.data_hvi, f.data_formatacao, COUNT(fa.id) as total_fardos
        FROM formatacoes f
